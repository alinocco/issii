"""
Точка входа для запуска экспериментов по задаче тревожности.
...
def run_all_experiments(X_train, y_train, X_val, y_val, X_test, y_test, feature_names):
    ...
    # Гипотеза 1: нелинейные ансамблевые модели (RF, XGBoost, LightGBM)
    print("\n" + "=" * 60)
    print("Гипотеза 1: Нелинейные ансамблевые модели")
    print("=" * 60)

    # RandomForest
    for n_est, depth in [(100, 10), (200, 15)]:
...
def main():
    parser = argparse.ArgumentParser(description="Запуск экспериментов ML для тревожности")
    parser.add_argument(
        "--model",
        type=str,
    ...
    df = load_dataset()
    X_train, X_val, X_test, y_train, y_val, y_test, feature_names = prepare_train_test_val(df)
